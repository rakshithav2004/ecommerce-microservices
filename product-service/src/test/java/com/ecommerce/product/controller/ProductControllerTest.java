...
class ProductControllerTest {
  ...
  void getProductById_shouldReturn200() throws Exception {
    ...
    when(productService.getProductById("product-001")).thenReturn(response);

    mockMvc
        .perform(get("/api/v1/products/product-001"))
  ...
  void getProductById_shouldReturn404WhenProductNotFound() throws Exception {
    ...
    when(productService.getProductById("invalid-id"))
        .thenThrow(new ProductNotFoundException("Product not found with id: invalid-id"));
  ...
  void getAllProducts_shouldReturn200() throws Exception {
    ...
    PageImpl<ProductResponse> page = new PageImpl<>(List.of(response), PageRequest.of(0, 10), 1);
    ...
    mockMvc
        .perform(
            get("/api/v1/products")
                ...
                .param("size", "10"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content[0].id").value("product-001"))
  ...
  void updateProduct_shouldReturn200() throws Exception {
    ...
    when(productService.updateProduct(eq("product-001"), any(ProductRequest.class)))
        .thenReturn(response);
    ...
    mockMvc
        .perform(
            put("/api/v1/products/product-001")
                ...
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.name").value("Samsung Galaxy Updated"))
        .andExpect(jsonPath("$.price").value(27000))
        .andExpect(jsonPath("$.stock").value(15));

    verify(productService).updateProduct(eq("product-001"), any(ProductRequest.class));
  }

  @Test
  void deleteProduct_shouldReturn204() throws Exception {

    doNothing().when(productService).deleteProduct("product-001");

    mockMvc.perform(delete("/api/v1/products/product-001")).andExpect(status().isNoContent());

    verify(productService).deleteProduct("product-001");
  }
  ...
  void reserveStock_shouldReturn200() throws Exception {
    ...
    when(productService.reserveStock("product-001", 2)).thenReturn(response);

    mockMvc
        .perform(put("/api/v1/products/product-001/reserve-stock").param("quantity", "2"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.stock").value(8));

    verify(productService).reserveStock("product-001", 2);
  }
  ...
  void releaseStock_shouldReturn200() throws Exception {
    ...
    when(productService.releaseStock("product-001", 2)).thenReturn(response);

    mockMvc
        .perform(put("/api/v1/products/product-001/release-stock").param("quantity", "2"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.stock").value(12));

    verify(productService).releaseStock("product-001", 2);
  }
